...
const EMPTY_ENTRY = { institution: '', degree: '', field_of_study: '', start_date: '', end_date: '', gpa: '' }
        ...
        <View key={entry.id || index} className="bg-white border border-gray-200 rounded-2xl mb-3 overflow-hidden">
          ...
          {expanded === index && (
            <View className="px-4 pb-4 border-t border-gray-100 pt-3">
              ...
              <FormField label="Field of Study" value={entry.field_of_study} onChangeText={v => update(index, 'field_of_study', v)} placeholder="Computer Science" />
              <View className="flex-row gap-3">
                <View className="flex-1">
                  <FormField label="Start" value={entry.start_date} onChangeText={v => update(index, 'start_date', v)} placeholder="2018" />
                </View>
                <View className="flex-1">
                  <FormField label="End" value={entry.end_date} onChangeText={v => update(index, 'end_date', v)} placeholder="2022" />
                </View>
                <View className="flex-1">
